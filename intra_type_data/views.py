...
def get_event_info_list(request):
    ...
    if request.method == 'GET':
        ...
        #search_keys存放datatable各列的模型字段
        #对于用外键连接的字段，需要用__具体字段来查询
        search_keys = ['aircraft_info__aircraft_serial_number',
                       'ATA__chapter',
                       'internal_number',
                       'attachment_info',
                       'corrective_action',
        ...
        for ls in result:
            data={
                ...
                "ATA": ls.ATA.chapter,
                "internal_number": ls.aircraft_owner,
                "attachment_info": ls.cumulative_flight_cycles,
                "corrective_action": ls.cumulative_flight_hours,
                "event_description": ls.event_description,
                "failure_number": ls.failure_number,
                "failure_part_name": ls.flight_character,
                ...
                "detail_info_source": ls.detail_info_source,
                "remarks": ls.remarks,
                }
            result_data.append(data)
        # 此时的key名字就是aaData，不能变
